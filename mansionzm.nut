...
function CreateInfectEffectOnPlayer(player) {
	...
	local infectRing = Entities.CreateByClassname("env_sprite");
	infectRing.__KeyValueFromInt("spawnflags", 2);
	infectRing.__KeyValueFromFloat("scale", 0.4);
	infectRing.__KeyValueFromInt("framerate", 60);
	infectRing.__KeyValueFromString("model","crossfire_infected/infected.vmt");

	local infectLight = Entities.CreateByClassname("env_sprite");
	infectLight.__KeyValueFromFloat("scale", 4.0);
	infectLight.__KeyValueFromInt("rendermode", 9);
	infectLight.__KeyValueFromString("rendercolor", "0 128 255");
	infectLight.__KeyValueFromString("model", "sprites/light_glow04_add_noz.vmt");

	local infectModel = Entities.CreateByClassname("prop_dynamic");
	infectModel.__KeyValueFromInt("spawnflags", 256);
	infectModel.__KeyValueFromInt("solid", 0);
	infectModel.__KeyValueFromInt("disableshadowdepth", 1);
	infectModel.__KeyValueFromInt("disableshadows", 1);
	infectModel.__KeyValueFromInt("rendermode", 1);
	infectModel.__KeyValueFromString("model", "models/cf/bluelight.mdl");

	infectRing.SetOrigin(playerOrigin);
	infectModel.SetOrigin(playerOrigin);
	infectLight.SetOrigin(playerOrigin);

	EntFireByHandle(infectRing, "showsprite", "", 0.00, player, player);
...
function Score()
{
	...
	textneirongct = format(ResolveString("GHOSTS_LEFT"), aaa, trenshu);
